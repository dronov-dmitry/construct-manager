...
class ErrorReportDialog extends StatefulWidget {
  ...
  /// Shows error dialog using a BuildContext (must have a Navigator ancestor).
  ...
  /// Shows error dialog using a Navigator's GlobalKey (works from global error handlers
  /// where no Navigator ancestor is available in the context).
  static Future<void> showWithGlobalKey(GlobalKey<NavigatorState> navigatorKey, {
    required Object error,
    StackTrace? stack,
    String? contextInfo,
  }) {
    ...
    if (navigator == null) {
      debugPrint('ErrorReportDialog: Navigator not available, error: $error');
      return Future.value();
    }
    navigator.push(
      MaterialPageRoute(
